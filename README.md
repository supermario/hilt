...
|            | Usage                                  | Description |
| -          | -                                      | -           |
| **exec**   | `events :: [Event] <- Postgres.exec db $ selectList [] []` | Runs a persistent query. See the [persistent guide](http://www.yesodweb.com/book/persistent). |
| **execR** | `events :: [Event] <- Postgres.execR db "SELECT * FROM events"` | Run a [postgresql-simple](https://hackage.haskell.org/package/postgresql-simple/docs/Database-PostgreSQL-Simple.html) query and decode the results to the specified type. |
| **execRP** | `events :: [Event] <- Postgres.execRP db "SELECT * FROM ?" ["events"]` | Run a paramaterised [postgresql-simple](https://hackage.haskell.org/package/postgresql-simple/docs/Database-PostgreSQL-Simple.html) query and decode the results to the specified type. |
